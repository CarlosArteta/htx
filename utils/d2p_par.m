function [P, beta] = d2p_par(D, u, tol)
%Same as d2p from t-SNEs toolbox with PARFOR
...
parfor i=1:n
  ...
  [H, thisP] = Hbeta(tempD(1, [1:i - 1, i + 1:end]), beta(i));
  
  % Evaluate whether the perplexity is within tolerance
  Hdiff = H - logU;
  tries = 0;
  while abs(Hdiff) > tol && tries < 50 %&& ~nanf
...
function [H, P] = Hbeta(Drow, beta)
P = exp(-Drow * beta);
sumP = sum(P);
H = log(sumP) + beta * sum(Drow .* P) / sumP;

function [net,stats] = cnn_train_dag_htx(net, imdb, getBatch, varargin)
%Slightly modified MatConvNet's CNN_TRAIN_DAG to support HTX's adversarial
%part
  ...
  for s=1:opts.numSubBatches
    ...
    if strcmp(mode, 'train')
      ...
      if net.meta.numPlates > 1
        if adverTurn
          net.layers(24).block.pos = 'open';
          net.layers(26).block.pos = 'close';
          net.layers(31).block.pos = 'open';
          net.params(31).learningRate = 1;
          net.params(31).weightDecay = 1;
          net.params(32).learningRate = 2;
        else
          net.layers(24).block.pos = 'close';
          net.layers(26).block.pos = 'open';
          net.layers(31).block.pos = 'close';
          net.params(31).learningRate = 0;
          net.params(31).weightDecay = 0;
          net.params(32).learningRate = 0;
        end
        adverTurn = ~adverTurn;
      end
  ...
  switch net.params(p).trainMethod
    ...
    case 'average' % mainly for batch normalization
      ...
      net.params(p).value = ...
          ...
          (thisLR/(batchSize*5)/net.params(p).fanout) * net.params(p).der ;
...
% -------------------------------------------------------------------------
sel = find(cellfun(@(x) isa(x,'GenericLoss'), {net.layers.block}) ...
  | cellfun(@(x) isa(x,'dagnn.Loss'), {net.layers.block})) ;
stats = struct() ;
for i = 1:numel(sel)
